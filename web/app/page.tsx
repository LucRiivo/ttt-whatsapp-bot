        ...
        <div className={styles.messages}>
          {messages.length === 0 && (
            <div className={styles.welcome}>
              ...
              <h2>Welcome to TTT Tax Bot</h2>
              <p>Ask me anything about South African tax, SARS procedures, eFiling, VAT, and more!</p>
              <div className={styles.suggestions}>
                <button onClick={() => setInput('What are the tax brackets for 2024?')}>
                  Tax brackets 2024
                </button>
                <button onClick={() => setInput('How do I apply for a tax clearance certificate?')}>
                  Tax clearance
                </button>
                <button onClick={() => setInput('When is the provisional tax deadline?')}>
                  Provisional tax
                </button>
              </div>
            </div>
                          ...
                          onClick={() => {
                            if (!isLoading) {
                              ...
                              fetch('http://localhost:3001/api/chat', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ message: btn.reply.title }),
                              })
                                .then(res => res.json())
                                .then(data => {
                                  setMessages((prev) => [...prev, { role: 'assistant', content: data.response, interactive: data.interactive }]);
              ...
              onChange={async (e) => {
                if (e.target.files && e.target.files[0]) {
                  ...
                  try {
                    ...
                    if (res.ok) {
                      setMessages(prev => [...prev, { role: 'assistant', content: `✅ Document uploaded: ${file.name}` }]);
                    } else {
